...
#Fauna
  ...
  NewEntriesFauna <- dataFaunaUM %>% filter(!is.na(dataFaunaUM[,"Pendaki.2020"]))
  #Subset new entries only
  namakolom_fauna <- colnames(NewEntriesFauna[15:34])
...
#Flora
  ...
  #Subset all entries in 2020
  NewEntriesFlora <- dataFloraUM %>% filter(!is.na(dataFloraUM[,"PENDAKI.2020"]))
  #Subset new entries only
  namakolom_flora <- colnames(NewEntriesFlora[16:27])
