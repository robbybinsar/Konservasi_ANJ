---
title: "| Indeks Keanekaragaman Region 3 \n| Jan - Mar 2021\n"
author: |
  | Robby B
...
div <- ggplot(data, aes(x=date, y=diversity_index, group = sites, color = sites )) +
    ...
    annotate(geom="text", x= as.Date("2021-01-17", "%Y-%m-%d"), y= 0.95 , 
             label="1 = Batas indeks keanekaragaman kategori rendah",size = 3)+
    annotate(geom="text", x = as.Date("2021-01-17", "%Y-%m-%d"), y=3.5, 
...
grafik <- ggplot(data, aes(x = date, y = species.number, fill = sites)) + 
    ...
    geom_text(aes(label= species.number),hjust=0.5, vjust=-1,size=3, 
              position = position_dodge(width = 7), color = "red")+
...
```{r h, fig.width=9, fig.height=6.5,echo=FALSE, fig.align='center', warning=FALSE}
anj <- readPNG("anj.png")
