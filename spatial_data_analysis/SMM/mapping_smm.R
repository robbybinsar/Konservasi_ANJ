...
# read data points
#priority_species <- read.csv("./spatial_data_analysis/SMM/plot_carbon.csv")
#priority_species$date <- as.Date(priority_species$date, format = "%d-%b-%y")
GRTT <- st_read(dsn = "./spatial_data_analysis/SMM/GRTT_POINT_2020/GROUND_CHECK_GRTT_JKE", layer = "GROUND_CHECK_GRTT_JKE")
...
#priority_buffer <- st_buffer(priority_sf, dist = 5, endCapStyle = "SQUARE")
...
tm_basemap(server = providers$Esri.WorldImagery)+
  ...
  tm_shape(GRTT) + tm_dots(col = "name", popup.vars = TRUE) +
